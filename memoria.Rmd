...
Una vez elaborado el modelo, realizamos una primera predicción con los datos _test_:

```{r, echo=FALSE}
data.frame(modelo = c("Modelo 2 (top 4)"),
           precision = c(0.9091),
           sensibilidad = c(0.9333), especificidad = c(0.9037),
           valor_pred_pos = c(0.6840), valor_pred_neg = c(0.9838))
```

Del mismo modo, __se mantiene en la "linea" de lo obtenido de otros modelos no lineales como _random forest_, _bagging_ o _gradient boosting___ (aunque si mejora los resultados con respecto a la _svm_ radial).
...
Por tanto, a la vista de los resultados, __no merece la pena emplear un modelo _boosting_ en lugar de _Random Forest__. Además, pese al mayor número de árboles que se han empleado (2000), una de las ventajas del modelo _Random Forest_, frente a _gbm_, _Xgboost_ o incluso _bagging_, __es el sorteo de variables y el menor tamaño de la submuestra__. De este modo, sorteando tan solo dos variables a lo largo de cada nivel del árbol (mtry = 2), así como tan solo 1000 observaciones para cada árbol (lo cual supone alrededor del 17 % del _sub-dataset_ empleado, menos del 50 % como es el caso de _gbm_), __los resultados obtenidos son muy similares, aún incluso con menos recursos__. 

No obstante, podemos comparar las medidas básicas de cada modelo (sensibilidad, especificidad, valor predictivo positivo y negativo):





En conclusión, __estudiamos el punto de corte con las predicciones de _Random Forest___.
